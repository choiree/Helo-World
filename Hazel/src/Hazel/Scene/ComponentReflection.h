...
namespace Hazel {
		...
		struct FieldMeta
		{
			...
			bool noSerialize = false;      // Skip during scene serialization
			bool noUI = false;             // Skip in editor property panel
		};
		...
		// for multiple attrs: .custom<FieldMeta>({.speed = 0.25f, .min = 0.0f, .max = 100.0f, .noSerialize = true})
		...
		#define HZ_FIELD_NOSERIALIZE        .custom<FieldMeta>({.noSerialize = true})
		#define HZ_FIELD_NOUI               .custom<FieldMeta>({.noUI = true})
		...
		// Variant that lets callers override the component traits.
		#define HZ_REGISTER_COMPONENT_EX(T, traits, ...) \
			static int _hz_reg_##T = []() -> int { \
				entt::meta_factory<T> factory{}; \
				factory.type(#T); \
