...
void sub_0804AAD4(void) {
    ...
    gTransitionState.trackedEntity = NULL;
    EraseAllEntities();
...
void Subtask_PortalCutscene_1(void) {
    ...
    UpdateEntities();
    if (gTransitionState.trackedEntity != NULL) {
        tmp = gRoomControls.scroll_y + 0x50 - gTransitionState.trackedEntity->y.HALF.HI - gTransitionState.trackedEntity->z.HALF.HI;
        if (tmp != 0) {
    ...
    DrawEntities();
    CopyOAM();
    if ((gTransitionState.completionFlag != 0) || sub_0804ACA8()) {
        gArea.filler3[0]++;
