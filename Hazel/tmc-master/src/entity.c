...
extern Manager gManagerPool;
void UpdatePlayerInput(void);
void ClearHitboxList(void);
void UpdateRoomTransitionPalette(void);
void ClearAllDeletedEntities(void);
void DeleteAllEntities(void);
void InitializeEntityLists(void);
...
void DarkNut_CheckPlayerRange(void) {
    s32 tilePos;
...
Manager* GetEmptyManager(void) {
    Temp* it;
    for (it = (Temp*)&gManagerPool; it < ((Temp*)&gManagerPool + 32); it++) {
        if (it->prev == 0) {
            return (Manager*)it;
        }
    }
...
void RestoreEntityLists(void) {
    MemCopy(&gEntityLists, &gEntityListsBackup, 0x48);
    InitializeEntityLists();
}

void BackupEntityLists(void) {
