...
void MinishPortalManager_Main(MinishPortalManager* this) {
    ...
    if (super->action == 0) {
        super->action = 1;
        this->unk_20 = this->unk_38 + gRoomControls.origin_x - PORTAL_OFFSET;
        this->unk_24 = this->unk_3a + gRoomControls.origin_y - PORTAL_OFFSET;
        return;
    }
    if (CheckPlayerProximity(this->unk_20, this->unk_24, 0x40, 0x40)) {
        gArea.portal_x = this->unk_20 + PORTAL_OFFSET;
        gArea.portal_y = this->unk_24 + PORTAL_OFFSET + gUnk_08107C6C[super->type];
