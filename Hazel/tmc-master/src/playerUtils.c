...
bool32 IsPointInRoom(u32, u32, RoomResInfo*);
...
u32 GetAdjacentRoomIndex(u32 param_1, u32 param_2, u32 param_3);
...
extern u16 gDefaultBgControlValues[];

void UpdateRoomBounds(void);
void ExpandMapDataToGrid(u16*, u32, u32);
void ExpandCollisionDataToGrid(u8*, u32, u32);
void ProcessSpecialTiles(void);
void InitializeRoomCollisionData(void);
void CreateCollisionDataBorderAroundRoom(void);
void CopyMapDataChunk(u16*, u16*);
void MergeCollisionLayers(void);
...
void UpdateActiveItems(PlayerEntity* this) {
    ...
    if (((animationFlags | gPlayerState.jump_status) & 0x80) == 0 && (gPlayerState.jump_status & 0x40) == 0 &&
...
bool32 sub_080782C0(void) {
    ...
    if (((gPlayerState.playerInput.newInput & INPUT_FUSE) != 0) &&
        ...
        if (gSave.kinstones.amounts[0] != 0) {
            ...
            gPlayerState.queued_action = PLAYER_KINSTONE_PROMPT;
        } else {
...
void sub_080792BC(s32 speed, u32 direction, u32 field_0x38) {
    gPlayerState.queued_action = PLAYER_LAUNCHED;
...
void sub_080792D8(void) {
    ...
    if (playerEntity->action == PLAYER_CUSTOM_ANIMATION || gPlayerState.dash_state || (u8)(gPlayerState.heldObject - 1) < 4 ||
        gPlayerState.jump_status || gPlayerState.floor_type == SURFACE_FF || animationFlags & 0x80 ||
...
void RespawnPlayer(void) {
    ...
    player->action = PLAYER_WAITFORSCROLL;
    player->z.WORD = 0;
    player->zVelocity = 0;
    player->knockbackDuration = 0;
    ResetPlayerPosition();
    if (!(gPlayerState.flags & PL_GYORG_FIGHT)) {
        if (gPlayerState.flags & PL_SPECIAL_MOVEMENT) {
    ...
    if ((gPlayerEntity.base.action != PLAYER_MINISHDIE) && (gPlayerEntity.base.health == 0)) {
        ...
        if (animationFlags == 0) {
            if (gPlayerState.swim_state != 0) {
                ...
                if ((itemGetTimer == 0) && ((gPlayerState.flags & PL_IN_MINECART) == 0)) {
                    ...
                    if (((gPlayerState.flags & PL_TRANSITION_STATE) == 0) && (sub_08079D48() == 0)) {
                        movementFlags |= 0x10;
                        return TRUE;
                    }
                }
...
bool32 sub_08079C30(Entity* player) {
    if (itemGetTimer == 0 && (gPlayerState.flags & PL_TRANSITION_STATE) == 0) {
        if ((gPlayerState.flags & PL_MINISH) != 0) {
            return TRUE;
        }
...
void sub_08079D84(void) {
    if ((gPlayerEntity.base.action == PLAYER_NORMAL) || (gPlayerEntity.base.action == PLAYER_ROLL)) {
        ...
        gPlayerState.queued_action = PLAYER_SCRIPTED_ACTION;
...
void ForcePlayerMovement(s32 speed, u32 direction) {
    if ((gPlayerEntity.base.direction & 4) == 0) {
...
bool32 PlayerCanBeMoved(void) {
    if ((gPlayerState.flags & (PL_BUSY | PL_DROWNING | PL_CAPTURED | PL_USE_PORTAL | PL_HIDDEN | PL_FROZEN |
        ...
        gPlayerEntity.base.action == PLAYER_CUSTOM_ANIMATION) {
        return FALSE;
    } else {
        return TRUE;
    }
...
void PlayerUpdateSwimming(Entity* this) {
    if ((((this->action != PLAYER_ROOMTRANSITION) || (movementFlags == 0)) &&
...
void SetRoomTileData(s32 basePosition, u32 layer, u32 width, u32 height) {
    u32 y;
    u32 x;
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
...
bool32 CanPlayerInteractWithEntity(Entity* this, u32 scrollDirection) {
    u32 room = GetAdjacentRoomIndex(this->x.HALF.HI, this->y.HALF.HI, scrollDirection);
    if (room != 0xff) {
...
u32 GetRoomBoundaryDirection(Entity* this, u32 direction) {
    u32 result = 0xff;
    switch (direction & 7) {
        case 0:
...
u32 GetAdjacentRoomIndex(u32 param_1, u32 param_2, u32 param_3) {
    ...
    index = 0;
    while (index < 0x40) {
        if (IsPointInRoom(param_1, param_2, ptr)) {
            return index;
        }
        index++;
        ptr++;
...
bool32 IsPointInRoom(u32 param_1, u32 param_2, RoomResInfo* info) {
...
void LoadRoomGfx(void) {
    ...
    if (gMapBottom.mapData[0] != 0xffff) {
        ExpandMapDataToGrid(gMapBottom.mapData, roomControls->width / 16, roomControls->height / 16);
        ExpandMapDataToGrid(gMapTop.mapData, roomControls->width / 16, roomControls->height / 16);
    ...
    if (!clearBottomMap) {
        InitializeRoomCollisionData();
    } else {
        ExpandCollisionDataToGrid(gMapBottom.collisionData, roomControls->width >> 4, roomControls->height >> 4);
        ExpandCollisionDataToGrid(gMapTop.collisionData, roomControls->width >> 4, roomControls->height >> 4);
        ProcessSpecialTiles();
    }
    ...
    if (!clearBottomMap) {
        ...
        CopyMapDataChunk(gMapDataBottomSpecial, &gMapDataBottomSpecial[0x2000]);
        MemClear(&gMapDataBottomSpecial[0x2000], 0x4000);
        MemCopy(&gMapDataTopSpecial, &gMapDataTopSpecial[0x2000], 0x4000);
        CopyMapDataChunk(gMapDataTopSpecial, &gMapDataTopSpecial[0x2000]);
    ...
    switch (roomControls->area) {
        ...
        case AREA_EZLO_CUTSCENE:
            if (gMapTop.bgSettings != NULL) {
                gMapTop.bgSettings->control = gDefaultBgControlValues[2];
            }
...
void CopyMapDataChunk(u16* dest, u16* src) {
    s32 index1;
    u32 index2;
    u16* ptr;
...
void ExpandCollisionDataToGrid(u8* data, u32 width, u32 height) {
    u8* ptr1;
    u8* ptr2;
    u32 index;
    u32 innerIndex;
...
void InitializeDiggingCaveEntrance(void) {
...
void RefreshRoomGraphics(void) {
    gRoomTransition.field2d = 2;
    LoadRoomGfx();
    gRoomTransition.field2d = 0;
}

void ExpandMapDataToGrid(u16* data, u32 width, u32 height) {
    u16* dst_ptr;
    u16* src_ptr;
    u16* dst_ptr_cpy;
    u16* src_ptr_cpy;
...
void LoadMapDataLayers(u32* dest) {
...
void LoadBitmapBackgroundData(u32* a1) {
    u32* v1; // r5@1
...
bool32 CheckGameCondition(s32 param_1, u32 param_2) {
    bool32 result = TRUE;
    switch (param_1) {
        case 0:
        default:
