...
extern Palette gPaletteStateTracker;
...
u32 FindNextUsedObjPalette(u32);
void MovePaletteData(u32, u32);
static void InitializeObjPaletteSlot(u32 a1);
...
void ResetObjPaletteTable(u32 a1) {
    u32 i;

    MemClear(gPaletteList, sizeof(gPaletteList));
    for (i = 0; i < 6; ++i) {
        InitializeObjPaletteSlot(i);
    }
    HandleRoomTransitionObjPalettes(a1);
}

void InitializeObjPaletteSlot(u32 index) {
...
void HandleRoomTransitionObjPalettes(u32 a1) {
    u32 tmp;
    ...
    if (a1) {
        CleanUpObjPalettes();
        InitializeObjPaletteSlot(0xf);
    } else {
        if (gPaletteStateTracker._0_0 == 4) {
            gPaletteStateTracker._0_0 = 0;
            gPaletteStateTracker._0_4 = 0;
            gPaletteStateTracker._1 = 0;
            gPaletteStateTracker.objPaletteId = 0;
        }
    }
}
...
void UnloadOBJPalette(Entity* entity) {
    ...
    u32 idx = *p;
    *p = 0;
    UpdatePaletteState(idx);
}

void UpdatePaletteState(u32 a1) {
    u32 uVar1;
    Palette* pPVar2;
    u32 uVar3;
...
void CleanUpObjPalettes(void) {
    ...
    iVar10 = 6;
    while ((iVar10 = FindNextUsedObjPalette(iVar10 + 1), iVar10 != 0 && (uVar4 = FindFreeObjPalette(1), uVar4 != 0xffffffff))) {
        if (iVar10 > (int)uVar4) {
            local_24[iVar10] = uVar4;
            MovePaletteData(iVar10, uVar4);
        }
    }
...
u32 FindNextUsedObjPalette(u32 a1) {
    u32 i;
    for (i = a1; i < 16; i++) {
...
void MovePaletteData(u32 a1, u32 a2) {
    Palette* pPVar1;
    Palette* pPVar2;
    s32 iVar2;
    u16* iVar3;
