...
extern void RefreshRoomGraphics(void);
extern void InitializeLighting(s32);
extern void LoadRoomGfx(void);
extern void LoadRoomTileSet(void);
extern void SetupSpecialMapData(void);
...
extern const u16 gBeanstalkSceneConfigs[]; // TODO figure out structure
...
extern const u32 gTileEntityCheckFuncs[]; // TODO actually function pointers?
extern const Data gBottomTileToEntityMap[];
extern const Data gTopTileToEntityMap[];

void CreateEntityFromTileData(const Data* entityData, u32 tilePos);
bool32 CanPlayerPush (void);
bool32 CreatePushedBlockAtPosition(Entity* this, u32 tilePos, u32 direction);
...
extern u32 GetRoomBoundaryDirection(Entity* this, u32 direction);
...
bool32 CanPushTileInDirection(MapLayer* mapLayer, u32 position, u32 collisionType);
bool32 CheckAndCreatePushableBlock(u32 param_1, u32 param_2, u32 param_3);
u32 CheckMultiPlayerPushBlock(Entity* this, u32 mode);
bool32 CanPushNormalBlock(MapLayer* mapLayer, u32 position, u32 collisionType);
bool32 CanPushSpecialTile(MapLayer* mapLayer, u32 position);
u32 GetPushableTilePosition(Entity* this, u32 direction);

void DispatchBeanstalkOverlay(void) {
    gUnk_080B4458[gMenu.overlayType]();
}
// 加载豆茎(Beanstalk)生长场景的辅助房间
void InitializeBeanstalkScene(void) {  
    Entity* beanstalkEntity;  // object  
    const u16* sceneConfig;   // ptr  
      
    sceneConfig = &gBeanstalkSceneConfigs[gUI.field_0x3 * 7];  
    SetLocalFlagByBank(FLAG_BANK_1, sceneConfig[2]);  
    LoadAuxiliaryRoom(sceneConfig[0], sceneConfig[1]);  
    gRoomControls.scroll_x += sceneConfig[3];  
    gRoomControls.scroll_y += sceneConfig[4];  
    gUpdateVisibleTiles = 1;  
    UpdateScrollVram();  
    gScreen.lcd.displayControl &= 0xfeff;  
    beanstalkEntity = CreateObject(BEANSTALK, 0, 0);  
    if (beanstalkEntity != NULL) {  
        beanstalkEntity->x.HALF.HI = gRoomControls.scroll_x + sceneConfig[5];  
        beanstalkEntity->y.HALF.HI = gRoomControls.scroll_y + sceneConfig[6];  
    }  
    LoadGfxGroup(0x11);  
    FlushSprites();  
    DrawEntities();  
    ...
    gTransitionState.stageIndex++;  
}

void BeanstalkSceneWaitFade(void) {
    if (gFadeControl.active == 0) {
        gTransitionState.stageIndex++;
        gTransitionState.timer = 0x186;
    }
}

void PlayBeanstalkGrowthAnimation(void) {
    FlushSprites();
    UpdateEntities();
    DrawEntities();
    CopyOAM();
    if ((--gTransitionState.timer == 0) || ((gInput.newKeys & (B_BUTTON | START_BUTTON)) != 0)) {
        gTransitionState.stageIndex++;
    }
}

void FinalizeBeanstalkAnimation(void) {
    MenuFadeIn(2, 0xb);
}

void SetBGDefaults(void) {
    gMapBottom.bgSettings = (BgSettings*)&gScreen.bg2;
    gMapBottom.bgSettings->control = gDefaultBgControlValues[0];
    gMapTop.bgSettings = &gScreen.bg1;
    gMapTop.bgSettings->control = gDefaultBgControlValues[1];
}
...
u32 UpdatePlayerCollision(void) {
    ...
    if (((direction & (DIR_NOT_MOVING_CHECK | 0x3)) == 0) && (movementFlags == 0)) {
        index = GetRoomBoundaryDirection(&gPlayerEntity.base, direction >> 2);
        if (index != 0xff && (gRoomControls.scroll_flags & 4) == 0) {
            ...
            if (GetCollisionDataAtTilePos(COORD_TO_TILE_OFFSET(&gPlayerEntity.base, -ptr1[0], -ptr1[1]),
                ...
                if ((((gPlayerState.flags & (PL_SPECIAL_MOVEMENT | PL_TRANSITION_STATE)) != 0) ||
                    ...
                    ((sub_080806BC(gPlayerEntity.base.x.HALF.HI - gRoomControls.origin_x,
                      ...
                      (((gPlayerState.heldObject != 0 || ((gPlayerState.gustJarState & 0xf) != 0)) ||
                        (CanPlayerInteractWithEntity(&gPlayerEntity.base, index) == 0)))))) {
                    return 3;
                }
    ...
    switch (direction) {
        case SPECIAL_TILE_0:
            if (CanPushNormalBlock(mapLayer, position, 2) == 0) {
                return 0;
            }
        ...
        case SPECIAL_TILE_27:
            if (CanPushTileInDirection(mapLayer, position, 0xb) == 0) {
                return 0;
            }
        ...
        case ACT_TILE_112:
            if ((gPlayerState.field_0x35 & 0x80) == 0) {
                if ((gPlayerEntity.base.frame & 1) != 0) {
                    if (CheckAndCreatePushableBlock(gPlayerState.field_0x35 << 2, tileType, position) != 0) {
                        return 1;
                    }
                }
            }
        ...
        case SPECIAL_TILE_11:
            if (CanPushTileInDirection(mapLayer, position, 8) == 0) {
                return 0;
            }
        ...
        case SPECIAL_TILE_90:
            if (CanPushTileInDirection(mapLayer, position, 2) == 0) {
                return 0;
            }
            // Move ice block.
        ...
        case ACT_TILE_114:
            ...
            position = CheckMultiPlayerPushBlock(&gPlayerEntity.base, 1);
            if (position == 0xffff) {
                return 0;
            }
            tmp2 = (position >> 0xc) - 1;
            ...
            while (index < 3) {
                if ((((*(u32*)(&(((GenericEntity*)gPlayerClones[0]))->field_0x6c)) & (1 << index)) != 0) &&
                    (CheckMultiPlayerPushBlock(gPlayerClones[index], 0) == position)) {
                    tmp3++;
                }
                index++;
            }
        ...
        case SPECIAL_TILE_43 ... SPECIAL_TILE_45:
            if (CanPushSpecialTile(mapLayer, position) != 0) {
        ...
        case SPECIAL_TILE_74:
            if (CanPushNormalBlock(mapLayer, position, 8) == 0) {
                return 0;
            }
        ...
        case SPECIAL_TILE_86:
            ...
            for (index = 0; index < 3; index++) {
                if (GetPushableTilePosition(gPlayerClones[index], 0) == position) {
        ...
        case SPECIAL_TILE_88:
            ...
            for (index = 0; index < 3; index++) {
                if (GetPushableTilePosition(gPlayerClones[index], 6) == position) {
                    SetTile(SPECIAL_TILE_89, position, gPlayerEntity.base.collisionLayer);
                    return 4;
                }
            }
...
bool32 TryGetPlayerFacingActTile(void) {
    u16 tileType;
    const s16* ptr;
...
bool32 CheckAndCreatePushableBlock(u32 direction, u32 tileType, u32 tilePos) {
    bool32 cond = FALSE;
    switch (tileType) {
        case 0x360:
            cond = TRUE;
    ...
    if (cond) {
        return CreatePushedBlockAtPosition(&gPlayerEntity.base, tilePos, direction);
    }
...
bool32 CreatePushedBlockAtPosition(Entity* this, u32 tilePos, u32 direction) {
    ...
    if (((mapLayer->collisionData[tilePos + gUnk_080B4488[direction >> 3]] == 0) ||
         ((u8)(mapLayer->collisionData[tilePos + gUnk_080B4488[direction >> 3]] - 0x20) < 0x20))) {
...
void LoadRoomTileEntities(void) {
    u32 tilePos;
    u32 width;
    u32 height;
    u32 indexX;
    ...
    for (indexY = 0; indexY < height; indexY++) {
        for (indexX = 0; indexX < width; indexX++, tilePos++) {
            for (entityData = gBottomTileToEntityMap; entityData->tileType != 0xffff; entityData++) {
                if (entityData->tileType == GetTileTypeAtTilePos(tilePos, 1)) {
                    if (gTileEntityCheckFuncs[entityData->conditionIndex] != 0) {
                        CreateEntityFromTileData(entityData, tilePos);
                        break;
                    }
                }
            }

            for (entityData = gTopTileToEntityMap; entityData->tileType != 0xffff; entityData++) {
                if (entityData->tileType == GetTileTypeAtTilePos(tilePos, 2)) {
                    if (gTileEntityCheckFuncs[entityData->conditionIndex] != 0) {
                        CreateEntityFromTileData(entityData, tilePos);
                        break;
                    }
...
void CreateEntityFromTileData(const Data* entityData, u32 tilePos) {
    ...
    if (entityData->kind != MANAGER) {
        entity = GetEmptyEntity();
        if (entity != NULL) {
            entity->kind = (u8)entityData->kind;
            entity->id = (u8)entityData->id;
            entity->type = (u8)entityData->type;
            entity->type2 = (u8)entityData->type2;
        ...
        if (manager != NULL) {
            manager->kind = (u8)entityData->kind;
            manager->id = (u8)entityData->id;
            manager->type = (u8)entityData->type;
            manager->type2 = (u8)entityData->type2;
...
bool32 AlwaysSpawnEntity(void) {
    return TRUE;
}

u32 SetupRoomVisuals(void) {
    return gRoomControls.scroll_flags >> 1 & 1;
}

void SetupRoomVisuals(bool32 loadGfx) {
    void (*func)(void);
