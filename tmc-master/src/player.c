...
static PlayerEntityAction PlayerLaunched;
...
static PlayerEntityAction PlayerSpecialMovement;
...
void DoPlayerAction(PlayerEntity* this) {
    static void (*const sPlayerActions[])(PlayerEntity*) = {
        ...
        [PLAYER_SPECIAL_MOVEMENT] = PlayerSpecialMovement,
...
static vo id PlayerNormal(PlayerEntity* this) {
    ...
    gPlayerState.pushedObject |= 0x80;
    if ((gPlayerState.flags & (PL_USE_OCARINA | PL_TRANSITION_STATE)) == 0) {
        UpdateFloorType();
    }
    ...
    sub_080792D8();
    if ((gPlayerState.jump_status | movementFlags) == 0 && (sub_08079550() || sub_08078F74(super))) {
        return;
    }
    DoJump(this);
    ...
    if (super->knockbackDuration == 0) {
        ...
        v13 = 0;
        if ((((animationFlags | super->direction) & DIR_NOT_MOVING_CHECK) | movementFlags) == 0 &&
            (animationFlags & 0x10) == 0) {
            v13 = 1;
            if (super->knockbackDuration == 0 &&
...
static void PlayerBounceUpdate(PlayerEntity* this) {
    ...
    if ((itemGetTimer == 0) && PlayerCheckNEastTile()) {
...
static void PlayerKinstonePrompt(PlayerEntity* this) {
...
static void PlayerCustomAnimation(PlayerEntity* this) {
...
static void sub_08071E04(PlayerEntity* this) {
    if ((super->z.WORD != 0) && (itemGetTimer == 0)) {
        UpdateFloorType();
...
static void PlayerScriptedAction(PlayerEntity* this) {
...
static void PlayerLaunched(PlayerEntity* this) {
...
static void PlayerRollUpdate(PlayerEntity* this) {
    ...
    if (!(gPlayerState.flags & PL_TRANSITION_STATE)) {
        UpdateFloorType();
    }
    if (CheckQueuedAction()) {
        gPlayerState.flags &= ~PL_ROLLING;
...
static void PlayerWaitForScroll(PlayerEntity* this) {
    ...
    if ((gPlayerState.flags & PL_TRANSITION_STATE) == 0) {
        UpdateFloorType();
    }
}
...
static void PlayerSpecialMovement(PlayerEntity* this) {
...
static void sub_08073924(PlayerEntity* this) {
    if (!(gPlayerState.flags & PL_ROLLING) && (super->z.HALF.HI & 0x8000) && !movementFlags) {
        gPlayerState.jump_status = 0x40;
        gPlayerState.direction = DIR_NONE;
        super->direction = DIR_NONE;
        PutAwayItems();
...
void sub_08073B8C(PlayerEntity* this) {
    ...
    if (super->z.HALF.HI >= -8) {
        if (!itemGetTimer && (sub_0807A2B8() || !sub_08079D48())) {
            COLLISION_ON(super);
            sub_08073B60(this);
            return;
        }
...
static void sub_08073C80(PlayerEntity* this) {
    ...
    if (gRoomTransition.player_status.spawn_type == PLAYER_MINISH) {
    ...
    if (!CheckQueuedAction()) {
        ...
        if (!CheckQueuedAction() && super->subAction != 2) {
            ...
            if (!UpdatePlayerCollision()) {
                ...
                if (!GravityUpdate(super, kGravityRate))
                    gPlayerState.jump_status = 0;
                if ((animationFlags & 0x80) == 0 && !movementFlags) {
                    if (super->iframes <= 8) {
                        if (gPlayerState.swim_state) {
                            gPlayerState.framestate = PL_STATE_SWIM;
                            PlayerSwimming(super);
                    ...
                    if ((gPlayerState.flags & PL_HIDDEN) == 0)
                        ...
                        gPlayerState.framestate = PL_STATE_HIDDEN;
                }
            }
        }
    }
...
static void sub_08074808(PlayerEntity* this) {
    ...
    if (GetInventoryValue(ITEM_FLIPPERS) == 1) {
        if (!gPlayerState.swim_state) {
            if ((gPlayerState.flags & PL_SPECIAL_MOVEMENT) != 0)
                gPlayerState.swim_state = 1;
            else
