...
Entity* CreateLinkAnimation(Entity* parent, u32 type, u32 type2) {
    ...
    if (e != NULL) {
        ...
        this->storeField7 = animationFlags;
        this->storeKeepFacing = gPlayerState.keepFacing;
        this->storeFieldA = movementFlags;
...
void CreateLargeWaterTraceWithRandomOffset(Entity* parentEntity, s32 minOffsetRadius, s32 maxOffsetRadius) {
    Entity* waterTraceEntity;  // 水迹特效实体
    s32 randomRadius;         // 随机偏移半径（在min和max之间）
    s32 randomAngle;          // 随机角度（控制偏移方向）

    waterTraceEntity = CreateLargeWaterTrace(parentEntity);  // 创建水迹特效
    if (waterTraceEntity != NULL) {
        randomAngle = Random();  // 获取随机值用于计算角度和半径
        // 计算随机半径（在[minOffsetRadius, maxOffsetRadius]范围内）
        randomRadius = (randomAngle % (maxOffsetRadius - minOffsetRadius + 1)) + minOffsetRadius;
        // 提取高16位作为角度索引（0-255范围，对应正弦表）
        randomAngle = randomAngle >> 0x10 & 0xff;
        // 计算X轴偏移（基于正弦值和随机半径）
        waterTraceEntity->x.WORD += FixedDiv(FixedMul(gSineTable[randomAngle], randomRadius << 8), 0x100) << 8;
        // 计算Y轴偏移（基于余弦值和随机半径，+0x40等价于90度偏移，即cos(angle)）
        waterTraceEntity->y.WORD -= FixedDiv(FixedMul(gSineTable[randomAngle + 0x40], randomRadius << 8), 0x100) << 8;
    }
}
