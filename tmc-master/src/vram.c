...
void UploadGFXSlotToVRAM(u32);
void ResetAllGFXSlots(void);
u32 FindFreeGFXSlots(u32);
void CleanUpGFXSlots(void);
void SetGFXSlotPalette(u32, u32);
void LinkEntityToGFXSlot(u32, Entity*, u32);
void SetGFXSlotStatus(GfxSlot*, u32);
u32 FindNextOccupiedGFXSlot(u32);
u32 FindFirstFreeGFXSlot(void);
void UpdateEntityGFXReferences(u32, u32);
...
void SetGFXSlotPalette(u32 index, u32 paletteIndex) {
...
void ResetAllGFXSlots(void) {
    ...
    gGFXSlots.vramUpdateMode = 0;
}

// Transfer gfx slot data to vram?
void UploadGFXSlotToVRAM(u32 index) {
    void* dest;
    GfxSlot* slot;
    ...
    if (slot->vramStatus != 0) {
        ...
        if (((slot->paletteIndex != 0xffff) && (slot->unk_3 != 0))) {
            ...
            for (loopIndex = 4; loopIndex > 0; loopIndex--) {
                if (ptr1->unk_00.unk2 != 0 && (gGFXSlots.vramUpdateMode != 0 || ptr1->unk_00.unk3 != 0)) {
                    ptr1->unk_00.unk3 = 0;
...
bool32 LoadFixedGFX(Entity* entity, u32 gfxIndex) {
    ...
    if (gfxIndex == 0) {
        ...
        for (index = 4; index < MAX_GFX_SLOTS; index++) {
            if (gfxIndex == gGFXSlots.slots[index].gfxIndex) {
                ...
                LinkEntityToGFXSlot(index, entity, GFX_SLOT_RESERVED);
                result = TRUE;
                return result;
            }
        }
    ...
    if (gfxIndex != 0) {
        ...
        ReserveGFXSlots(index, gfxIndex, count);
        SetGFXSlotPalette(index, gFixedTypeGfxData[gfxIndex]);
    _080ADFF2:
        LinkEntityToGFXSlot(index, entity, GFX_SLOT_RESERVED);
    }
    return TRUE;
...
bool32 LoadSwapGFX(Entity* entity, u32 count, u32 slotIndex) {
    ...
    LinkEntityToGFXSlot(slotIndex, entity, status);
_080AE058:
    return slotIndex != 0;
}
...
void LinkEntityToGFXSlot(u32 index, Entity* entity, u32 status) {
    GfxSlot* slot;
    ...
    if (gGFXSlots.unk0 != 0) {
        for (occupiedIndex = 4; (occupiedIndex = FindNextOccupiedGFXSlot(occupiedIndex)) != 0; occupiedIndex++) {
            ...
            if (firstFreeIndex <= occupiedIndex) {
                UpdateEntityGFXReferences(occupiedIndex, firstFreeIndex);
...
// Swap gfx
void UpdateEntityGFXReferences(u32 param1, u32 param2) {
