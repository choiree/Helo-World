...
void ResetFortressOfWindsState(void);
...
bool32 CanDispEzloMessage(void) {
    ...
    if ((gPlayerEntity.base.z.HALF.HI & 0x8000) && !movementFlags)
        return FALSE;
...
void LoadAuxiliaryRoom(u32 area, u32 room) {
    LoadRoomData(area, room);
    gRoomControls.camera_target = NULL;
    CloneMapData();
    InitializeCamera();
}

void LoadRoomData(u32 area, u32 room) {
...
void InitializeMinecartData(void) {
...
void SavePlayerRoomStatus(void) {
...
void HandleAreaTransition(void) {
    if (HasLocationChanged()) {
        switch (gRoomControls.area) {
            ...
            case AREA_CAVE_OF_FLAMES:
                InitializeMinecartData();
                break;
            case AREA_OUTER_FORTRESS_OF_WINDS:
                ResetFortressOfWindsState();
                break;
            default:
                ResetTimerFlags();
                break;
